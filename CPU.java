...
public class CPU extends AbstractY86CPU.Sequential {
    ...
    public final static int A_IADDQ          = 0x0;
    public final static int A_ISUBQ          = 0x1;
    public final static int A_IANDQ          = 0x2;
    public final static int A_IXORQ          = 0x3;

    // Enhanced function codes for I_OPL and I_IOPL
    public final static int A_IMULQ          = 0x4;
    public final static int A_IDIVQ          = 0x5;
    public final static int A_IMODQ          = 0x6;
	...
	try {
		...
		case I_JXX:
		    ...
		    break;
		case I_CALL:			
			if (f.iFn.getValueProducedInt()==0) 
				f.valC.set (mem.readLongUnaligned (F.pc.get()+1));
			break;
		default:
		    f.valC.set (0);
	...
	if (E.stat.getInt()==S_AOK) {
	    ...
	    switch (aluFun) {
	    case A_ADDQ:
	    	if ( E.iCd.getInt() == 5 ||  E.iCd.getInt() == 4) {
	    		if (E.iFn.getInt() == 4) {
	    			e.valE.set (aluB * 8 + aluA);
	    		}
	    		else e.valE.set (aluB + aluA);
	    	}
	    	else {
